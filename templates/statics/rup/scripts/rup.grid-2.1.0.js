		...
		_init : function(args) {
			if (args.length > 1) {
				...
				settings.gridComplete = function(){
					...
					$("#"+$(this).attr("id")+" [title]").rup_tooltip({show:{delay:1000}});
				};
...
//							}
					...
					,onSelectAll_default: function(aRowids, status){
						var selectMsg = $.rup.i18nParse($.rup.i18n.base,"rup_grid.selectMsg"),
							...
							if (status) {//si hay que seleccionar
								var elementosRestantes = Number(self.rup_grid("getGridParam", "records") - self.rup_grid("getGridParam", "reccount") - $.data(jqGrid[0], "selectedRowsCont")),
								...
								selectedRecords = $.data(jqGrid[0], "selectedRowsCont") + self.rup_grid("getGridParam", "reccount") - $.data(jqGrid[0], "rowsSelectedPage");
							} else {
								var elementosRestantes = Number($.data(jqGrid[0], "selectedRowsCont") - self.rup_grid("getGridParam", "reccount"));
				...
				if(!settings.hasMaint ){
					/* HACK IE */
					if ($.rup.browser.isIE){
						self[0].rup_gridProps.windowWidth=$(window).width();
					}
					/* FIN HACK IE */
					$(window).resize(function(){
						/* HACK IE */
						if(!$.rup.browser.isIE || ($.rup.browser.isIE && self[0].rup_gridProps.windowWidth!==$(window).width())){
							if ($.rup.browser.isIE){
								self[0].rup_gridProps.windowWidth=$(window).width();
							}
							/* FIN HACK IE */
							...
							var newWidth = $(settings.fluidBaseLayer).width()-settings.offset,
								oldWidth = settings.gridWidth;
							if (newWidth <= oldWidth){
								//Redimensionar
								$(grid).setGridWidth(newWidth);
							} else {
								//Tamaño inicial
								$(grid).setGridWidth(oldWidth);
							}
						}
					});
					...
					settings.gridWidth = settings.width;
					
					//Si el espacio de visualización es menor menor que el tamaño de la tabla se redimensiona
					var elementWidth = $(settings.fluidBaseLayer).width()-settings.offset;
					if (elementWidth <= settings.gridWidth){
						$(grid).setGridWidth(elementWidth);
					}
				}
				...
				*/
				...
				$('#gview_' + self[0].id + ' thead th[title]:last').rup_tooltip({
					position: {
						my: 'top right',
						at: 'bottom right'
					}
