...
(function ($) {
		...
		return this.each(function () {
			...
			case $.rup.dialog.AJAX:
				...
				if (settings.showLoading) { //si hay que mostrar la capa de cargando por defecto a false
					$.blockUI({
						message: '<img src="' + RUP + '/basic-theme/images/rup.ajaxLoader.gif" style="position:absolute;top:12px;left:15px;" alt="' + $.rup.i18n.rup_blockUI.cargando + '" ><h1 class="loading">' + $.rup.i18n.rup_blockUI.cargando + '...' + '</h1>'
					});
				}
				...
				if (!settings.url || settings.url === null || settings.url === '') {
					$.rup.msgAlert({title: $.rup.i18n.rup_global.error, message: $.rup.i18n.rup_dialog.noURL});
					return false;
				}
				...
				$.ajax({
					...
					error: function (XMLHttpRequest, textStatus, errorThrown) { //en caso de error mostramos un mensaje de alerta
						$.unblockUI();
						$(document).rup().msgAlert({message: $.rup.i18n.rup_dialog.errorLoadingData});
					}
				});
				break;
			}
			...
			if (settings.buttons && settings.buttons !== null) { //controlar que existan los botones
				...
				if (btnsLength > 1 && linkButtonsLength === 0 && settings.rupCheckStyle) {
					$(document).rup().msgAlert({message: $.rup.i18n.rup_global.rupCheckStyleError});
			...
			if (!created) { //si ha sido creado no hace falta volver a añadir el elnace de cierre
				$("#" + id).dialog(settings);
				closeSpan = "<span id='closeText_" + id + "' style='float:right;font-size:0.85em;'>" + $.rup.i18n.rup_global.cerrar + "</span>";
				aClose = $("<a href='#'></a>")
				.attr("role", "button")
	...
	$.extend($.fn.rup_dialog, { 
		...
		createBtnLinks : function (btn, id) {
			...
			.html(btn.text)
			.click(btn.click);
			$('div[aria-labelledby=ui-dialog-title-' + id + '] .ui-dialog-buttonset ').prepend(buttonHREF);
		}
	});
