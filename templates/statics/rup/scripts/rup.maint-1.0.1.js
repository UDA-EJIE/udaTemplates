...
(function ($) {
	...
	$.fn.rup_maint("extend", {
		...
		cancelEditing : function () {//cancela la edición de una linea
			...
			this[0].prop.btnNew.button("option", "disabled", false);
		...
		onBeforeEdit : function (rowN) {//evento donde crear los objetos de RUP
			...
			$("#" + mntName)[0].prop.btnCancel.button("option", "disabled", false);
			$("#" + mntName)[0].prop.btnNew.button("option", "disabled", true);
			...
			$("input[name='" + lastColName + "']", relatedGrid).bind("keydown", function(event) {	
				...
				if (event.keyCode == 9) { // TAB
					if (!event.shiftKey) {
						...
						// Se realiza el guardado del registro de la tabla
						relatedGrid.rup_grid('saveRow', rowN, rup_maint.saveEditable, "clientArray", null, rup_maint.aftersavefunc, rup_maint.saveEditableError, null);

						// Se comprueba si se han producido errores en el proceso de guardado
						if(!hasErrors){
							...
							var rowID=dataIds[rowNumber+1];

							if (rowNumber+1 == relatedGrid.rup_grid("getDataIDs").length) {//si es la ultima fila hay que paginar y poner la primera en edicion
								...
								if (parseFloat(page) + 1 <= numPag) {
									...
									var dataIds = relatedGrid.rup_grid("getDataIDs");
									var rowID=Number(dataIds[0]);
								} else {
									return false;
								}
							}

							// Comprobamos si estamos en modo edicion
							if ($("#" + mntName)[0].prop.MODO==="new"){
								...
								$("body").data("clicktimer" , window.setTimeout(function () {
									$(relatedGrid[0]).find("tr.addElement:first .editable:first").focus();
								}, 0));
							}else{
	...
	//							relatedGrid.rup_grid("setSelection", Number(rowN) + 1, false);
								relatedGrid.rup_grid("setSelection", rowID, false);
								
								$("#" + mntName).rup_maint("editElement", rowID);
								
								// Situamos el focus en el primer campo editable de la línea en edicion
								$("body").data("clicktimer" , window.setTimeout(function () {
									$("# tr " +  + " .editable:first", relatedGrid).focus();
								}, 0));
							}
						}else{
							
							// Si se han producido errores de validacion se mantiene el edicion la linea actual
							hasErrors=false;
							$("body").data("clicktimer" , window.setTimeout(function () {
								$("#" + Number(rowN) + " .editable:last", relatedGrid).focus();
							}, 0));
							return false;
						}
						...
						if(!hasErrors){
							if (rowNumber == 0) {//si es la ultima fila hay que paginar y poner la primera en edicion
								if (parseFloat(page)> 1) {
									...
									var dataIds = relatedGrid.rup_grid("getDataIDs");
									var rowID=Number(dataIds[dataIds.length-1]);
...
//							relatedGrid.rup_grid("setSelection", Number(rowN-1), false);
							...
							$("#" + mntName).rup_maint("editElement", rowID);
							$("body").data("clicktimer" , window.setTimeout(function () {
								$("#" + rowID + " .editable:last", relatedGrid).focus();
							}, 0));
						}else{
							hasErrors=false;
							$("body").data("clicktimer" , window.setTimeout(function () {
								$("#" + Number(rowN) + " .editable:first", relatedGrid).focus();
							}, 0));
							return false;
						}
		...
		restore: function () {//evento que se lanza al restaurar la fila.
			...
			maint[0].prop.btnNew.button("option", "disabled", false);
		...
		aftersavefunc : function (rowid, res) {//Evento que se lanza cuandos e termina de editar
			if (res) {
				$("#" + $.data(this, "maintName")).rup_maint("saveMaint", null, rowid);
			}
		},
		checkOutOfGrid : function (evt, obj) {
			if (evt.target.id === "") {
				if(evt.target.className!=='' && $("#gbox_"+this[0].prop.jQueryGrid[0].id).find("." + evt.target.className).length > 0) {
					return false;
				...
				if ($("#gbox_"+this[0].prop.jQueryGrid[0].id).find("#" + evt.target.id).length > 0 ) {
					return false;
				}
			}
...
//						this.prop.btnNew.button("option", "disabled", true);
...
//									} else {//si ha habido cambiso mostrar el mensaje de confirmacion
										...
										// FIXME: Buscar solucion para el control de errores
										if (hasErrors){
											this.prop.jQueryGrid.rup_grid("editRow", this.prop.lastsel, true, rup_maint.onBeforeEdit, 
													rup_maint.saveEditable, "clientArray", null, rup_maint.aftersavefunc, null, 
													rup_maint.restore);
											this.prop.jQueryGrid.rup_grid("setSelection", this.prop.lastsel, null);
											hasErrors=false;
											return false;
										}
...
//									}
		...
		saveMaint : function (saveAndRepeat, rowId) {
			function getRowidFromPk(p, id) {
				var cont = 1;
				for (var i = 0 ; i < mnt.prop.selectedRows.length; i++) {//me recorro las paginas
					if (mnt.prop.selectedRows[i] === "p_" + p) {//si es la pagina
			...
			return this.each(function () {	
				...
				if (mant.prop.jQueryGrid.rup_grid("isEditable")) {
					...
					mant.prop.btnNew.button("option", "disabled", false);
					
					if (rowId==null){
						rowId=mant.prop.jQueryGrid.rup_grid("getSelectedRows")[0];
					}
					dt = mant.prop.jQueryGrid.rup_grid("getRowData", rowId);//$.toJSON(data);
...
//									mant.prop.MODO = null;
									...
									if (mant.prop.jQueryGrid.rup_grid("isMultiselect")) {
										...
										mant.prop.jQueryGrid.rup_grid("setRowData", aCurrentRow[1].split("id_")[1], xhr);
									} else {
									...
									if (errors.length === 2) {//comprobamos que tenemos 2 elementos el primero es el modelObject que a causado el error y el segundo es el objecto con los errores que ha causado
										...
										$.each(causedErrors, function (key, value) {
											...
											if (!mant.prop.jQueryGrid.rup_grid("isEditable")) {
												...
												var colNames = mant.prop.jQueryGrid.rup_grid("getColModel");
												
												var encontrado = false;
												for (var i=0;i<colNames.length && !encontrado;i=i+1){
													if (colNames[i].name==key){
														errorKey = colNames[i].label;
														encontrado=true;
													}
												}
												if (!encontrado){
...
//					mant.prop.btnNew.button("option", "disabled", false);
	...
	$.fn.rup_maint("extend", {
		_init : function(properties) {
								...
								if (xhr.status === 406) {//si ha habido algun error en las validaciones...
									...
									if (errors.length === 2) {//comprobamos que tenemos 2 elementos el primero es el modelObject que a causado el error y el segundo es el objecto con los errores que ha causado
										...
										$.each(causedErrors, function (key, value) {
											...
											//si existe el icono no se lo añadimos otra vez
											if ($("#" + key).parent().find(".rup-maint_validateIcon").length === 0) {
												$("#" + key).parent().append("<img class='rup-maint_validateIcon' title='" + errMsg + "' src='" + t[0].prop.imgPath + "/exclamation.png'>");
			...
			function createData(detailDiv,detaiBody) {//Se crea el formulario de detalle haciendo uso del colModel del grid
				...
				$(obj.p.colModel).each(function (i) {
						...
						if (nm !== 'cb' && this.editable === true) {
							...
							elc = createEl(this.edittype, opt, null, false, $.extend({}, $.jgrid.ajaxOptions, obj.p.ajaxSelectOptions || {}));
			...
			// var colModel=self[0].prop.jQueryGrid.rup_grid("getGridParam","colModel");
			
//			debugger;
//			$.each(colModel,function(key,value){
//				if (value.name===self[0].prop.primaryKey){
//					if(!value.key){
//						colModel[key].key=true;
//					}
//				}
//			});
//			
//			self[0].prop.jQueryGrid.rup_grid("setGridParam","colModel",colModel);
//			
//			debugger;
			...
			//if (settings.searchForm !== null) { //Si tenemos formulario de busqueda actualizamos los datos a enviar al servidor para añadir los campos de busqueda.
				settings.jQueryGrid[0].p.serializeGridData = function (postData) {
					...
					//lo de diferente de 1 es porque la primera vez y cada vez que se le pulsa al buscar vamos a la pagina uno con lo que hay que lanzar los criterios
//					if ((Number($(".ui-pg-input",this.p.pager).val())>postData.page || Number($(".ui-pg-input",this.p.pager).val())<postData.page) && postData.page!=1) {
//						return postData;
//					}
