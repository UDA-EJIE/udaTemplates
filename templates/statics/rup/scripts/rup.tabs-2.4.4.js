...
(function ($) {
	...
	$.fn.rup_tabs("extend",{
		...
		addTab : function(args){
			...
			if (args.lengthLiteral!==undefined && args.label.length>args.lengthLiteral){
	...
	$.fn.rup_tabs("extend",{
			...
			_parseJSON : function (json, json_i18n, tabs, pos, profundidad, settings) { 
				...
				//limitar la longitud del literal del titulo del tab
				if (settings.lengthLiteral!==undefined){
					
					$.each(json_i18n, function( index, element ) {
						if (json_i18n[index].length>settings.lengthLiteral)
							json_i18n[index]=json_i18n[index].substr(0,settings.lengthLiteral).concat("...");
					});
				}
