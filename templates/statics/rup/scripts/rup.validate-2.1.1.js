...
(function ($) {
	...
	$.fn.rup_validate("extend",{
		...
		resetForm:function(){
			...
			if (settings!=null && settings.showErrorsInFeedback){
	...
	$.fn.rup_validate.defaults = {
			...
			feedbackErrorConfig:{
				...
				getFieldName: function(self, form, field){
					var ruptype = field.attr("ruptype"), labelForName, labelElem;
					...
					labelElem = field.parent().find("label[for='"+labelForName+"']:not(."+self.settings.errorClass+")");
