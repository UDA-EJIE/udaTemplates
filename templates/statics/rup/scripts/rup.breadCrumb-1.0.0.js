...
(function ($) {
	$.widget("$.rup_breadCrumb", {
		...
		_create: function () {
			...
			breadCrumbSpan = $("<span>").addClass("rup-breadCrumbs_span").text($.rup.i18n.rup_breadCrumb.youAre),
			ulBreadCrumb = $("<ul>").attr("id", "rup_breadCrumbs_ul").addClass("rup-breadCrumb_main"), breadCrumbStruct = null,
			lastCrum = null;
			...
			ulBreadCrumb.append(this._createLI($.rup.i18n.rup_breadCrumb.start, $.rup.CTX_PATH));
			...
			lastCrum.replaceWith($("<label>").text(lastCrum.text()).css({ "font-weight":"bold", "color": "#333333"}));
			delete lastCrum;
		...
		_createBreadCrumb : function (breadCrumbStruct, elem, parentUl) {//nos recorremos la entrada correspondiente
			...
			if (breadCrumbStruct.i18nCaption) {//si tengo i18nCaption es que es elemento final
				createdLI = this._createLI($.rup.i18n.app[this.element.attr("id")][breadCrumbStruct.i18nCaption], (breadCrumbStruct.url ? $.rup.CTX_PATH+breadCrumbStruct.url : "#"));
			} 
			...
			if (breadCrumbStruct.subLevel) {
				...
				for (var i = 0; i < breadCrumbStruct.subLevel.length; i++) {
					...
					subLevelUL.append(this._createLI($.rup.i18n.app[this.element.attr("id")][breadCrumbStruct.subLevel[i].i18nCaption], (breadCrumbStruct.subLevel[i].url ? breadCrumbStruct.subLevel[i].url : "#")));
				}
