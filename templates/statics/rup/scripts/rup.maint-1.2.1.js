		...
		updateDetailPagination : function (rowPos, totalElements) {
			...
			return this.each(function () {
				$("#rup_maint_selectedElements").html("" + 
						rowPos + " " +
						$.rup.i18nParse($.rup.i18n.base,"rup_maint.de") + " " + 
						totalElements + " " + 
						$.rup.i18nParse($.rup.i18n.base,"rup_maint.elements"));		
			...
			return this.each(function () {
				...
				$.rup_ajax({
					...
					success: function (xhr, ajaxOptions) {
						...
						objSerializedForm = $.map(mnt.prop.detailForm.serializeArrayNull(), function(elem,i){
							if(xhrArray[elem.name]){
							    return {name:elem.name,value:xhrArray[elem.name]};
							}else{
...
//						if (!mnt._checkValidRowId_editline(id)){
//							return false;
//						}
...
//				}
				if ((mant.prop.jQueryGrid.rup_grid("isEditable") && !$.rup_utils.compareObjects(init,dt)) 
						|| ( !mant.prop.jQueryGrid.rup_grid("isEditable") && (mant.prop.detailForm && mant.prop.detailForm.serialize() !== mant.prop.detailForm.data('initialData')))) {
					dt = $.toJSON(dt);
					var errores = false;
					
					mant.prop.detailForm.ajaxSubmit({
		...
		toggleSearchForm : function (capa) {//Apertura/Cierre del formulario de busqueda
			return this.each(function () {
				if (this.prop.searchForm === null) {//si no hay formulario de busqueda no hacemos nada
					...
					var searchString = "", temp = "", aux, i, fieldName, fieldValue, searchForm, primero, field, fieldId, fieldValue, labelLabelId, ruptype, parentLabel;
					if ($("#" + capa).is(":hidden")) {
						$("#" + capa+" div").slideDown("slow");
						$("#" + capa).show("slow");
						$('#titleSearch_' + this.prop.name).text($.rup.i18nParse($.rup.i18n.base,"rup_maint.searchOptions"));
						...
						for (i = 0; i < aux.length; i++) {
							if (aux[i].value !== "") {
								...
								if (field){
									...
									labelLabelId;
									ruptype=field.attr("ruptype");
									
									var tagType=field.get(0).tagName;
									if (tagType==='SELECT'){
										fieldValue=" = "+$("option[value='"+aux[i].value+"']",field).html();
									}else if (tagType==='INPUT' && field.attr("type")==="checkbox"){
										fieldValue="";
									}else{
									...
									if(ruptype){
										if(ruptype==='combo'){
											fieldId=fieldId+'-button';
										}
									}	
									
									parentLabel = $("label[for='" + fieldId + "']",searchForm);
									
									if (parentLabel){
										fieldName = parentLabel.html();
									}else{
										fieldName= $("[name='" + aux[i].name + "']",searchForm).parent().find('div').html();
									}
...
//						$('#titleSearch_' + this.prop.name).append("<i>" + searchString + "</i>");
						$('#titleSearch_' + this.prop.name).html($.rup.i18nParse($.rup.i18n.base,"rup_maint.searchOptions")+"<i>" + searchString + "</i>");
					}
	...
	$.fn.rup_maint("extend",{
		...
		showFieldValidationErrors: function(responseText){
			...
			return this.each(function () {	
				var mant=this, errorTXT = $.rup.i18nParse($.rup.i18n.base,"rup_maint.validateError"), errors = null, errorKey = null, detailFormName, preMode,
				errors = eval("(" + responseText + ")");
				if (errors.length === 2) {//comprobamos que tenemos 2 elementos el primero es el modelObject que a causado el error y el segundo es el objecto con los errores que ha causado
					...
					$.each(causedErrors, function (key, value) {
						...
						if ($("[name='" + key+"']",mant.prop.detailForm).parent().find(".rup-maint_validateIcon").length === 0) {
							$("[name='" + key+"']",mant.prop.detailForm).parent().append("<img class='rup-maint_validateIcon' title='" + errMsg + "' src='" + mant.prop.imgPath + "/exclamation.png'>");
						}
...
//			if (!$(this).hasClass("ui-state-disabled")) { 	
				if (prop.detailForm.serialize() !== prop.detailForm.data('initialData')) {
					$.rup_messages("msgConfirm", {
...
//			}
		...
		_createDetailForm: function (withUserForm) {//				
			...
			if (withUserForm) {//si no han indicado formulario de detalle
				...
				$("#" + prop.detailForm[0].id + " .datepicker").each(function () {
					$(this).rup_date();
				});
				
				$("#" + prop.detailForm[0].id + " .numeric").each(function () {
					$(this).numeric(",");
				});
				
				$("#" + prop.detailForm[0].id + " .integer").each(function () {
					$(this).numeric(false);
				});
		...
		_createDetailNavigation: function () {
			...
			return "<div id='rup-maint_detailPagination' style='border-bottom:1px solid #D1D1D1'><div id='pag_" +
					prop.name + "' style='float:right'><span id='last_" + 
					prop.name + "' alt='" + $.rup.i18nParse($.rup.i18n.base,"rup_maint.last") +
					"' class='rup-maint_linkPaginacionDetalle'>" + $.rup.i18nParse($.rup.i18n.base,"rup_maint.last") + 
					"</span><span id='forward_" + prop.name + "' alt='" + $.rup.i18nParse($.rup.i18n.base,"rup_maint.next") + 
					"' class='rup-maint_linkPaginacionDetalle'>" + $.rup.i18nParse($.rup.i18n.base,"rup_maint.next") +
					"</span><span id='back_" + prop.name + "' alt='" + $.rup.i18nParse($.rup.i18n.base,"rup_maint.previous") + 
					"' class='rup-maint_linkPaginacionDetalle'>" + $.rup.i18nParse($.rup.i18n.base,"rup_maint.previous") +
					"</span><span id='first_" + prop.name + "' alt='" + $.rup.i18nParse($.rup.i18n.base,"rup_maint.first") + 
					"' class='rup-maint_linkPaginacionDetalle'>" + $.rup.i18nParse($.rup.i18n.base,"rup_maint.first") + "</span></div>";
		},
		/*
			...
			if (prop.jQueryGrid.rup_grid("isEditable")) {
				...
				if (prop.MODO === "new") {
					...
					if (!jqGrid.rup_grid("isMultiselect")) {
						...
						prop.toolbar.enableButton("edit");
						prop.toolbar.enableButton("delete");
					}
	...
	$.fn.rup_maint("extend", {
		_init : function(properties) {
			...
			if (!jqGrid.rup_grid("isEditable")) {
				...
				prop.detailDiv.rup_dialog({type: jQuery.rup.dialog.DIV, autoOpen: false, modal: true, width: 569, specificLocation: prop.name, create: prop.eventCreateDetailForm, 
					open: function () {
						//Gestor de cambios
//						t[0].prop.detailForm.data('initialData', t[0].prop.detailForm.serialize());
			...
			if (prop.searchForm !== null) {
				...
				lnkClean = $("<a>").attr("id", "clean_search_" + prop.name).attr("class", "rup-enlaceCancelar").text($.rup.i18nParse($.rup.i18n.base,"rup_global.clean")).bind("click", function () {
			...
			if (jqGrid.rup_grid("isMultiselect")) {
				...
				jqGrid[0].rup_gridProps.onAfterSelectAll = function (aRowids, select, fromGridComplete) {
					return self._onAfterSelectAll_multiselect(aRowids, select, fromGridComplete);
				};
				
				jqGrid[0].rup_gridProps.onDeSelectAllRows = function (self, select) {
					return self._onDeSelectAllRows_multiselect(self, select);
				};
			...
			//Tratamiento para comportamiento de aspecto fluído.
			if(jqGrid[0].rup_gridProps.fluidBaseLayer!==null){
				//Con cada cambio de tamaño de ventana ajustamos el contenido.
				$(window).resize(function(){
					...
					$("#"+prop.name).children().width($("#"+prop.name).width()-prop.fluidOffset);
				});
			}
	}
	});
	...
	$.fn.rup_maint("extend",{
		...
		deselectRow : function(rowid){
			...
			if (self._isSelected(rowid,null,page,rowNum)){
				...
				prop.selectedRows["p_" + page].splice(prop.selectedRows["p_" + page].indexOf(rowNum),1);
//				delete prop.selectedRows["p_" + page]["l_"+rowNum]["id_" + rowid];
	 ...
	 * _onAfterSelectAll_multiselect(aRowids, select, fromGridComplete):
	 * _onDeSelectAllRows_multiselect(self, select):
	...
	$.fn.rup_maint("extend",{
		...
		_markSelectedRows_multiselect:function(){
			...
			var self = this, prop = self[0].prop, jqGrid = prop.jQueryGrid, dataIDs, page, pageStored=false;
		...
		_onAfterSelectRow_multiselect: function (rowid, select) {
			...
			var self = this, prop = self[0].prop, jqGrid = prop.jQueryGrid, page, selected, selectesElem = [], ind, selectedTotalRows;
...
//			selectedTotalRows = self.rup_maint("getTotalSelectedRowNum");
		...
		_onAfterSelectAll_multiselect : function(aRowids, select, fromGridComplete){
		...
		_onDeSelectAllRows_multiselect : function(self, select){
