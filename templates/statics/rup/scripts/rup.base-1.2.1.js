		...
		i18nParse : function (properties, i18nCaption, defaultValue){
			if (i18nCaption !== undefined && i18nCaption !== null && i18nCaption !== ""){
				...
				for (var i = 0; i < i18nCaptionArr.length; i++) { //Navegar por las ramas JSON
					//if (!$.isEmptyObject(i18nValue) && !$.isEmptyObject(i18nValue[i18nCaptionArr[i]])){
		...
		iniRup : function () {
			...
			if (cookie !== null && cookie !== "") {//si tenemos cookie con el lenguaje
				if ($.inArray(cookie,$.rup.AVAILABLE_LANGS_ARRAY)!==-1){
					...
					//retrocompatibilidad (UDAInterceptor genera correctamente la cookie, pero en versiones anteriores no)
				...
				//$.rup._avoidRUPFails();
				//return false;
				this.lang = (navigator.language || navigator.userLanguage).split("-")[0].toLowerCase();
				var options = { path:"/", domain: document.domain};
				//$.rup_utils.setCookie($.rup.LOCALE_COOKIE_NAME, this.lang, options);
			}
