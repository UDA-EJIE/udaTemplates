...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name.replace(/\./g, "_")+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+"'"+ tooltip+">";
			idn = ts.p.colModel[i].index || ts.p.colModel[i].name;
			thead += "<div id='jqgh_"+ts.p.colModel[i].name.replace(/\./g, "_")+"' "+tdc+">"+ts.p.colNames[i];
