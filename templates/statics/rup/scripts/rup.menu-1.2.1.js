...
(function ($) {
			...
			this.element.find("ul").andSelf().menu({
				...
				select: function (event, ui) {
					if (ui.item.context.href !== '') {
						if (ui.item.context.target === "_blank" || ui.item.context.target === "_new"){
							window.open(ui.item.context.href,jQuery(this).attr('title'));
						} else {
	...
	$.rup_menu.extend({
		parseJSON_vertical: function (json, json_i18n, menu, force) {
				...
				if((element.pathUrl !== undefined) || (element.url !== undefined)){
					...
					if (element.newWindow !== true){
						menu.prepend($('<li>').append(
							$('<a>').text($.rup.i18nParse(json_i18n,element.i18nCaption)).attr('href', objectUrl).css('text-decoration', 'underline')
						));
					} else {
						menu.prepend($('<li>').append(
							$('<a>').text($.rup.i18nParse(json_i18n,element.i18nCaption)).attr('href', objectUrl).attr('target',"_blank").css('text-decoration', 'underline')
						));
					}	
				} else {
		...
		parseJSON_horizontal: function (json, json_i18n, menu, force){
			...
			for (var i = json.length; i--; ) {
				...
				if((element.pathUrl !== undefined) || (element.url !== undefined)){
					...
					if (element.newWindow !== true){
						menu.prepend($('<div>').addClass('rup_div_menu').append($('<a>').text($.rup.i18nParse(json_i18n,element.i18nCaption)).attr('href', objectUrl)));
					} else {
						menu.prepend($('<div>').addClass('rup_div_menu').append($('<a>').text($.rup.i18nParse(json_i18n,element.i18nCaption)).attr('href', objectUrl).attr('target',"_blank")));
					}	
				} else {
    ...
    $.fn.rup_menu = function (properties) {
		...
		return this.each ( function(index, element) {		
			...
			if (settings.json === undefined){
				json = settings.menu; 
				json_i18n = (settings.i18nId === undefined)? $.rup.i18n.app[settings.id]:$.rup.i18n.app[settings.i18nId];
			} else {
				json = settings.json;
			...
			/** MENU HORIZONTAL **/
				...
				$(this).parseJSON_horizontal(json, json_i18n, $('#'+settings.id),settings.forceAbs);
				//Procesar botones
			...
			/** MENU VERTICAL */
				...
				//Generar estructura de menu
				$(this).parseJSON_vertical(json, json_i18n, $('#'+settings.id),settings.forceAbs);
			...
			if(settings.display !== 'horizontal' && settings.interLevel ===0){
				if (contentMV.size() === 1){
					...
					$.rup_messages("msgError", {
						title: $.rup.i18nParse($.rup.i18n.base,"rup_menu.developerError"),
						message: "<p>"+$.rup.i18nParse($.rup.i18n.base,"rup_menu.structuralError")+"</p>"
					});
				}
			}
